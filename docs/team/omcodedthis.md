...
Designed `SKUList.java`, `SKU.java`, `SKUTaskList.java`, and `SKUCommandHandler.java` for core inventory management of
...
files blocking directory creation), autonomously generates directories, and guarantees safe I/O stream closure,
rigorously tested in `ExportTest.java`.
...
structured, queryable object (Moderate Difficulty). Normalizes unpredictable inputs and utilizes
`Collections.unmodifiableMap` to strictly enforce a safe, read-only data flow.
...
hierarchy (including `MissingArgumentException.java`, `SKUNotFoundException.java`, and `InvalidCommandException.java`) (
Moderate Difficulty). Enforced clean separation of concerns in `CommandRunner.java` by translating nested domain errors
into user-friendly UI messages without leaking stack traces, supported by extensive tests like `CommandHandlerTest.java`,
enabling 68.6% test coverage.
